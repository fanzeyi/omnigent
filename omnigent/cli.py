...
# object registered under a second name). Kept runnable/registered but
# omitted from the ``--help`` listing so the alias isn't shown as a
# duplicate line.
    ...
    def format_commands(self, ctx: click.Context, formatter: click.HelpFormatter) -> None:
        ...
        for subcommand in self.list_commands(ctx):
            ...
            if cmd is None or cmd.hidden:
                continue
            # Skip pure aliases (same Click object under a second name, e.g.
            # ``update`` -> ``upgrade``) so the listing doesn't show a
            # duplicate line; the alias stays registered and runnable.
