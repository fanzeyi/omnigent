...
def _warn_legacy_path(legacy_env: str, canonical_env: str) -> None:
    ...
    _logger.warning(
        "%s is deprecated; set %s instead. %s support will be "
        "removed in %s.",
        legacy_env,
        canonical_env,
        legacy_env,
...
def _parse_override_entry(
    ...
    if not isinstance(value, dict):
        ui.warn(
            f"config `harness.{key}` must be a mapping, got "
            f"{type(value).__name__} — ignoring it."
        )
        return None
    ...
    if command is not None:
        if isinstance(command, str) and command.strip():
            ...
            ui.warn(
                f"config `harness.{key}.command` must be a non-empty string "
                "— ignoring it."
            )
    ...
    if args is not None:
        if isinstance(args, list) and all(isinstance(a, str) for a in args):
            ...
            ui.warn(
                f"config `harness.{key}.args` must be a list of strings "
                "— ignoring it."
            )
    return parsed
