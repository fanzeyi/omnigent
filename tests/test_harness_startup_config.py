...
def test_command_explicit_flag_wins(
    ...
    assert (
        resolve_harness_command(
            "codex", default="codex", explicit="/explicit/codex", cfg=cfg
        )
        == "/explicit/codex"
    )
...
def test_command_falls_back_to_default(
    ...
    monkeypatch.delenv("OMNIGENT_CODEX_PATH", raising=False)
    assert (
        resolve_harness_command("codex", default="codex", explicit=None, cfg={})
        == "codex"
    )
...
def test_command_canonical_id_for_native_harness(
    ...
    assert (
        resolve_harness_command(
            "codex-native", default="codex", explicit=None, cfg={}
        )
        == "/env/codex-native"
    )
...
def test_command_alias_resolves_to_canonical_env_var(
    ...
    assert (
        resolve_harness_command("claude", default="claude", explicit=None, cfg={})
        == "/env/claude"
    )
...
def test_command_empty_explicit_falls_through(
    ...
    assert (
        resolve_harness_command("codex", default="codex", explicit="   ", cfg={})
        == "/env/codex"
    )
...
def test_resolve_harness_path_legacy_warns_only_once(monkeypatch: pytest.MonkeyPatch, caplog) -> None:
    ...
    warns = [r for r in caplog.records if "HARNESS_CODEX_PATH" in r.message and "deprecated" in r.message]
    assert len(warns) == 1
...
def test_resolve_harness_path_ignores_non_registry_legacy_var(monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_args_config_base_then_cli() -> None:
    ...
    assert resolve_harness_args(
        "codex", ("--dangerously-skip-permissions",), cfg=cfg
    ) == ["--config", "k=v", "--dangerously-skip-permissions"]
